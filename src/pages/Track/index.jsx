...
export default function Track() {
    ...
    const handleGetUserSignedInfo = async () => {
        try {
            const { data: userTracks } = await api.get(
                `/user/tracks/${user_id}`
            );
            ...
            const { data: trackData } = await api.get(
                `/track/${track_id}/contents`
            );

            const { data: getUserProgress } = await api.get(
                `/user/${user_id}/${track_id}/progress`,
                { user_id }
            );
    ...
    const DynamicContentContainer = ({
        ...
        return (
            <>
                {contentType === "Video" ? (
                    <VideoContainer trackInfo={trackInfo} />
                ) : (
                    <ArticleContainer
                        trackInfo={trackInfo}
        ...
        <>
                ...
                {trackInfo?.userSigned ? (
                    <>
                        <DynamicContentContainer
                            contentType={
                                trackInfo?.trackContent[trackInfo?.current]
                                    ?.type
                            }
                            trackInfo={trackInfo}
                            setTrackInfo={setTrackInfo}
                        ...
                        <ContentListContainer
                            handleGetUserSignedInfo={handleGetUserSignedInfo}
                            trackInfo={trackInfo}
                            setTrackInfo={setTrackInfo}
                        />
                    </>
            ...
            <div className="flex flex-col items-center px-8 gap-4 my-[120px]">
                <DiscordText className="text-center text-xl flex flex-col ">
                    <strong>Ficou com dúvidas?</strong> Entra lá na comunidade
                    do Discord que a gente te ajuda!
                </DiscordText>
